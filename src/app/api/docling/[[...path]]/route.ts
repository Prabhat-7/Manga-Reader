...
async function proxyRequest(
  ...
  if (!baseUrl) {
    return Response.json(
      { error: "DOCLING_BASE_URL is not configured." },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Docling proxy error.";
    return Response.json(
      { error: "Could not reach Docling API server.", detail: message },
      { status: 502 },
    );
  }
}
