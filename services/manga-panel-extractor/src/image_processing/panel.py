...
def sort_panel_items_by_manga_order(items: list[tuple[tuple[int, int, int, int], object]]) -> list[tuple[tuple[int, int, int, int], object]]:
    """
    Sort items in manga reading order:
    top-to-bottom rows, and right-to-left inside each row.
    """
    if len(items) < 2:
        return items

    heights = [bb[3] for bb, _ in items]
    row_tolerance = max(1.0, float(np.median(heights)) * 0.5)

    # Start by vertical position so rows are discovered top-to-bottom.
    pending = sorted(items, key=lambda item: (item[0][1] + item[0][3] / 2, item[0][0]))

    rows: list[list[tuple[tuple[int, int, int, int], object]]] = []
    ...
    for item in pending:
        ...
        for i, row_center in enumerate(row_centers):
        ...
        if best_row == -1:
            ...
            row_centers[best_row] = float(np.mean([tmp_bb[1] + tmp_bb[3] / 2 for tmp_bb, _ in rows[best_row]]))

    ordered: list[tuple[tuple[int, int, int, int], object]] = []
    for row_idx in sorted(range(len(rows)), key=lambda i: row_centers[i]):
        row = rows[row_idx]
        ordered.extend(sorted(row, key=lambda item: item[0][0], reverse=True))

    return ordered


def sort_contours_by_manga_order(contours: list[np.ndarray]) -> list[np.ndarray]:
    contour_items = [(cv2.boundingRect(contour), contour) for contour in contours]
    return [contour for _, contour in sort_panel_items_by_manga_order(contour_items)]


def sort_bounding_boxes_by_manga_order(
    bounding_boxes: list[tuple[int, int, int, int]]
) -> list[tuple[int, int, int, int]]:
    bounding_box_items = [(bounding_box, bounding_box) for bounding_box in bounding_boxes]
    return [bounding_box for _, bounding_box in sort_panel_items_by_manga_order(bounding_box_items)]


def get_contour_group_bounding_box(contours: list[np.ndarray]) -> tuple[int, int, int, int]:
    bounding_boxes = [cv2.boundingRect(contour) for contour in contours]
    x = min(bb[0] for bb in bounding_boxes)
    y = min(bb[1] for bb in bounding_boxes)
    right = max(bb[0] + bb[2] for bb in bounding_boxes)
    bottom = max(bb[1] + bb[3] for bb in bounding_boxes)
    return (x, y, right - x, bottom - y)


def sort_contour_groups_by_manga_order(grouped_contours: list[list[np.ndarray]]) -> list[list[np.ndarray]]:
    groups_with_bounding_boxes = [(get_contour_group_bounding_box(group), group) for group in grouped_contours]
    return [group for _, group in sort_panel_items_by_manga_order(groups_with_bounding_boxes)]


def get_bounding_box_group_bounding_box(
    bounding_box_group: list[tuple[int, int, int, int]]
) -> tuple[int, int, int, int]:
    x = min(bb[0] for bb in bounding_box_group)
    y = min(bb[1] for bb in bounding_box_group)
    right = max(bb[0] + bb[2] for bb in bounding_box_group)
    bottom = max(bb[1] + bb[3] for bb in bounding_box_group)
    return (x, y, right - x, bottom - y)


def sort_bounding_box_groups_by_manga_order(
    grouped_bounding_boxes: list[list[tuple[int, int, int, int]]]
) -> list[list[tuple[int, int, int, int]]]:
    groups_with_bounding_boxes = [
        (get_bounding_box_group_bounding_box(group), group)
...
def threshold_extraction(
    ...
    contours = sort_contours_by_manga_order(contours)
...
def generate_panel_blocks(
    ...
    def get_panels(contours):
        ordered_contours = sort_contours_by_manga_order(contours)
        panels = extract_panels(image, ordered_contours, mode=mode)
    ...
    if merge == MergeMode.NONE:
        ...
        grouped_contours = sort_contour_groups_by_manga_order(grouped_contours)
...
def generate_panel_blocks_by_ai(image: np.ndarray, merge: str = MergeMode.NONE) -> list[np.ndarray]:
    ...
    def get_panels(bounding_boxes):
        panels = []
        ordered_bounding_boxes = sort_bounding_boxes_by_manga_order(bounding_boxes)
        for x, y, w, h in ordered_bounding_boxes:
    ...
    if merge == MergeMode.NONE:
        ...
        grouped_bounding_boxes = sort_bounding_box_groups_by_manga_order(grouped_bounding_boxes)
        for group in grouped_bounding_boxes:
            panels.append(adaptive_hconcat(get_panels(group)))
    elif merge == MergeMode.VERTICAL:
